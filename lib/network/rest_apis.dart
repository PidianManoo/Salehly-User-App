...
Future<List<ShopListData>> getShopList({
  int page = 1,
  required List<ShopListData> shopListData,
  Function(bool)? lastPageCallback,
  String latitude = '',
  String longitude = '',
  String search = '',
  ...
  String longitudes = longitude.isNotEmpty ? 'longitude=$longitude&' : '';
  try {
    ShopListModel res = ShopListModel.fromJson(await handleResponse(
        await buildHttpResponse(
            'shop-list?$searchPara$providerIds$latitudes$longitudes$pages$perPages',
            method: HttpMethodType.GET)));
    if (page == 1) shopListData.clear();
    shopListData.addAll(res.data.validate());

    lastPageCallback?.call(res.data.validate().length != PER_PAGE_ITEM);
    cachedShopList = shopListData;
    appStore.setLoading(false);
  } catch (e) {
    appStore.setLoading(false);
    throw e;
...
Future<ShopDetailModel> getShopDetailApi({int? shopId}) async {
  String sId = shopId != null ? "$shopId" : '';
  ShopDetailModel res = ShopDetailModel.fromJson(await handleResponse(
      await buildHttpResponse('shop-detail/$sId', method: HttpMethodType.GET)));
  return res;
}
