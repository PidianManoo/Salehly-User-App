...
InputDecoration inputDecoration(BuildContext context,
  ...
  return InputDecoration(
    ...
    hintText: hintText,
    hintStyle: secondaryTextStyle(fontFamily: saudiRiyalsFontFamily),
...
Widget trailing({double? size, Color? color}) {
  return ic_arrow_right.iconImage(size: size ?? 16, color: color ?? null);
}
...
// Future<void> showInAppUpdateFlow({required BuildContext context}) async {
//   final result = await PlayxVersionUpdate.showInAppUpdateDialog(
//     context: context,
//     type: PlayxAppUpdateType.immediate,
//     // iosOptions: PlayxUpdateOptions(newVersion: "3.0.4"),
//     iosUiOptions: PlayxUpdateUIOptions(
//       displayType: PlayxUpdateDisplayType.dialog,
//       isDismissible: !appStore.isUserAppForceUpdate,
//       title: (i) => "It's Time to Update!",
//       description: (info) =>
//           'A new version of the app (${info.newVersion}) is available. '
...
//     success: (isShown) {
//       if (isShown) {
//         log('In-app update dialog process initiated or no update needed.');
//       }
//     },
//     error: (error) async {
//       if (error is PlayxInAppUpdateCanceledError &&
//           appStore.isUserAppForceUpdate) {
//         print("The user said 'no' or 'cancel' the update.");
//         showInAppUpdateFlow(context: context);
//       }
//       log('Error during in-app update dialog: ${error.message}');
//     },
//   );
// }
...
void toastSaudiRiyals(
  ...
  if (value.validate().isEmpty || isLinux) {
    ...
    Fluttertoast.showToast(
      msg: value.validate(),
      gravity: gravity,
      fontAsset: "assets/fonts/saudiriyalsymbol.otf",
      toastLength: length,
...
bool get isSaudiRiyalsSymbol =>
    (appConfigurationStore.currencySymbol == saudiRiyalsOldSymbol ||
        appConfigurationStore.currencySymbol == saudiRiyalsNewSymbol);

String? get saudiRiyalsFontFamily {
  return isSaudiRiyalsSymbol ? SAUDI_ARABIA_CURRENCY_SYMBOL_FONT_STYLE : null;
}
...
List<LanguageDataModel> filterDefaultLanguages(List<String> defaultLanguages) {
  ...
  if (filteredList.isNotEmpty) {
    filteredList.sort((a, b) {
      if (a.languageCode == 'ar') return -1; // pt should be first
