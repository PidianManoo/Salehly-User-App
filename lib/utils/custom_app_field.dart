...
/// Blocks Arabic script when the app language is English, and blocks Latin
/// characters when the app language is Arabic.
class LanguageInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    final langCode = appStore.selectedLanguageCode;
    if (langCode == 'en' && _arabicRegex.hasMatch(newValue.text)) {
      return oldValue;
    }
    if (langCode == 'ar' && _latinRegex.hasMatch(newValue.text)) {
      return oldValue;
    }
    return newValue;
  }
...
class _CustomAppTextFieldState extends State<CustomAppTextField> {
  ...
  /// Returns the combined list of formatters, automatically injecting
  /// [LanguageInputFormatter] for natural-language field types.
  List<TextInputFormatter> _buildInputFormatters() {
    final existing = widget.inputFormatters ?? const [];
    // Email, password, phone, number, and URL fields should never be
    // language-restricted — users must type ASCII regardless of locale.
    if (widget.textFieldType == TextFieldType.EMAIL ||
        ...
        widget.textFieldType == TextFieldType.URL) {
      return existing;
    }
    return [...existing, LanguageInputFormatter()];
  }
