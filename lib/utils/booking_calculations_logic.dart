...
BookingAmountModel finalCalculations({
  ...
  num? vatPercentage,
  num? platformCommissionPercentage,
}) {
  ...
  data.vatPercentage = vatPercentage.validate();
  data.vatAmount = calculateVatAmount(data.finalSubTotal + totalExtraCharges, data.vatPercentage);

  data.platformCommissionPercentage = platformCommissionPercentage ?? appConfigurationStore.platFormCommissionEarningPercentage.toDouble();
  data.totalPlatformCommissionAmount = calculateTotalPlatformCommissionAmount(data.finalSubTotal, data.platformCommissionPercentage);

  data.finalGrandTotalAmount = (data.finalSubTotal + data.finalTotalTax + data.vatAmount + totalExtraCharges + data.totalPlatformCommissionAmount).toStringAsFixed(appConfigurationStore.priceDecimalPoint).toDouble();
...
num calculateTotalPlatformCommissionAmount(num subTotal, [num? commissionPercentage]) {
  num rate = commissionPercentage ?? appConfigurationStore.platFormCommissionEarningPercentage;

  return (subTotal * rate / 100).toDouble();
}

num calculateVatAmount(num subTotal, num vatPercentage) {
  if (vatPercentage == 0) return 0;

  return (subTotal * vatPercentage / 100).toStringAsFixed(appConfigurationStore.priceDecimalPoint).toDouble();
}
