...
// import 'package:booking_system_flutter/screens/service/component/service_faq_widget.dart'; // FAQ section — disabled for now.
...
class _ServiceDetailScreenState extends State<ServiceDetailScreen>
  ...
  Widget providerWidget({required UserData data}) {
    // BookingDetailProviderWidget already renders its own cardColor
    // background — wrapping it in another cardColor container here would
    // double the padding and (in dark mode) stack a second border with no
    // visible separation, since both use the exact same fill color.
    return Container(
      width: context.width(),
  ...
  // FAQ section — disabled for now, not in use.
  ...
  Widget build(BuildContext context) {
    Widget buildBodyWidget(AsyncSnapshot<ServiceDetailResponse> snap) {
      if (snap.hasError) {
        // Without an AppBar here, a network failure left the user stuck on
        // this screen with no way back except the OS back gesture.
        return AppScaffold(
          appBarTitle: '',
          child: NoDataWidget(
              ...
              Expanded(
                child: AnimatedScrollView(
                  ...
                  children: [
                          ...
                          Row(
                            children: [
                              Icon(Icons.access_time_rounded,
                                  size: 15, color: appTextSecondaryColor),
                              6.width,
                          ...
                          children: [
                            ...
                            if (snap.data!.serviceDetail!.discount.validate() >
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: defaultActivityStatus.withValues(
                                      alpha: 0.12),
                                  borderRadius: radius(20),
                                ),
                                child: Text(
                    ...
                    (snap.data!.provider!.email == null &&
                            snap.data!.provider!.displayName == null)
                        ? SizedBox()
