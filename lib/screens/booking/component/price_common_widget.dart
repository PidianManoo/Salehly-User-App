...
class PriceCommonWidget extends StatelessWidget {
  ...
  final PostJobData? postRequestDetail;
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                if (bookingDetail.totalPlatformCommissionAmount.validate() != 0)
                  Column(
                    children: [
                      ...
                      Row(
                        children: [
                          Row(
                            children: [
                              Text(platformCommissionLabel, style: secondaryTextStyle(size: 14)).expand(),
                            ],
                          ).expand(),
                          16.width,
                          PriceWidget(price: bookingDetail.totalPlatformCommissionAmount.validate(), color: Colors.red, isBoldText: true),
                        ],
                      ),
                    ],
                  ),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                /// VAT applied on the booking amount (dynamic, applies to every booking type)
                if (bookingDetail.vatAmount.validate() != 0)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text(language.vatLabel(bookingDetail.vatPercentage.validate()), style: secondaryTextStyle(size: 14)).expand(),
                          16.width,
                          PriceWidget(price: bookingDetail.vatAmount.validate(), color: Colors.red, isBoldText: true),
                        ],
                      ),
                      12.height,
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            8.width,
                            Text(
                          language.saudiVatNote(bookingDetail.vatPercentage.validate(), bookingDetail.platformCommissionPercentage.validate()),
                ...
                if ((bookingDetail.isHourlyService || bookingDetail.isFixedService))
                  Column(
                    ...
                    children: [
                      ...
                      /// Urgent priority fee (10% of total amount)
                      if (postRequestDetail?.isUrgentBooking == true) ...[
                        10.height,
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Row(
                              children: [
                                ...
                                4.width,
                                Text(
                                  language.urgentPriorityFeeLabel,
                                  style: secondaryTextStyle(size: 14, color: urgentColor),
                                ),
                              ],
                            ).flexible(fit: FlexFit.loose),
                            PriceWidget(
                              price: (bookingDetail.totalAmount.validate() * 0.10).round(),
                              color: urgentColor,
                              isBoldText: true,
                            ),
                          ],
                ...
                /// total platform commission (shown once per booking)
                if (bookingDetail.totalPlatformCommissionAmount.validate() != 0)
                Column(
                  children: [
                    Divider(height: 26, color: context.dividerColor),
                ...
                // ── Urgent extra charges row ─────────────────────────────
                if (postRequestDetail?.isUrgentBooking == true &&
                    postRequestDetail?.extraCharges.validate().isNotEmpty == true) ...[
                  16.height,
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Row(
                        children: [
                          Icon(Icons.bolt_rounded, size: 14, color: urgentColor),
                          4.width,
                          Text(language.extraCharges, style: secondaryTextStyle(size: 14, color: urgentColor)),
                        ],
                      ).expand(),
                      PriceWidget(
                        price: double.tryParse(postRequestDetail!.extraCharges.validate()) ?? 0,
                        color: urgentColor,
                        isBoldText: true,
                      ),
                    ],
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          PriceWidget(
                            price: bookingDetail.bookingType.validate() == BOOKING_TYPE_USER_POST_JOB
                                ? postJobGrandTotal
                                : bookingDetail.totalAmount.validate(),
                            color: primaryColor,
                          ),
                        ],
                      ),
  ...
  num get postJobGrandTotal => bookingDetail.amount.validate() + bookingDetail.vatAmount.validate() + bookingDetail.totalPlatformCommissionAmount.validate();

  String get platformCommissionLabel => bookingDetail.platformCommissionPercentage.validate() != 0
      ? '${language.platformCommission} (${bookingDetail.platformCommissionPercentage.validate().toStringAsFixed(0)}%)'
