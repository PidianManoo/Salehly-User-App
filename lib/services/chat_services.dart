...
class ChatServices extends BaseService {
  ...
  Future<void> setUnReadStatusToTrue({required String senderId, required String receiverId, String? documentId}) async {
    ...
    // Mark only messages RECEIVED by senderId (from receiverId) as read
    // Path: messages/{senderId}/{receiverId}/ — the current user's copy
    final QuerySnapshot unread = await ref!
        .doc(senderId)
    ...
    for (final doc in unread.docs) {
      batch.update(doc.reference, {'isMessageRead': true});
    }
  ...
  Stream<int> getUnReadCount({required String senderId, required String receiverId, String? documentId}) {
    // Count unread messages in the current user's copy: messages/{senderId}/{receiverId}/
    // Only messages where receiverId == senderId (i.e., received by the current user)
    return ref!
        .doc(senderId)
        .collection(receiverId)
        .where('isMessageRead', isEqualTo: false)
