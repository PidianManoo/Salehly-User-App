    ...
    if (await TheAppleSignIn.isAvailable()) {
      ...
      switch (result.status) {
        case AuthorizationStatus.authorized:
          ...
          if (result.credential != null && user.email.validate().isNotEmpty) {
            ...
            if (firstName.isEmpty || firstName == "") {
              String emailPrefix = user.email.splitBefore("@");
              // Split at first special character (., _, -) and take first part
              firstName = emailPrefix.split(RegExp(r'[._\-]')).first;
              lastName = emailPrefix.splitAfter(RegExp(r'[._\-]'));
            }

            await setValue(APPLE_EMAIL, result.credential?.email ?? user.email);
            await setValue(APPLE_GIVE_NAME,
                result.credential?.fullName?.givenName ?? firstName);
            await setValue(APPLE_FAMILY_NAME,
                result.credential?.fullName?.familyName ?? lastName);
          } else {
